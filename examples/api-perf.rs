    ...
    fn job(&self) {
        ...
        socket
            .write_message(Message::Text(
                "{\"id\":1,\"jsonrpc\":\"2.0\",\"method\":\"chain_getBlock\",\"params\":[]}".into(),
            ))
            .unwrap();
...
impl HttpRequest {
    pub fn Rpc(&self, url: &str, params: String) -> bool {
        ...
        }
        {
            ...
            if let Err(_e) =
                transfer.read_function(|buf| Ok(params.as_bytes().read(buf).unwrap_or(0)))
            {
                return false;
            }
...
impl Work for HttpRequest {
    fn job(&self) {
        ...
        self.Rpc(
            "http://8.210.110.126:7003/Polkadot/2754a6ab77be92d8338e9d0ed2f63585",
