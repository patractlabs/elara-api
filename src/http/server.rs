    ...
    fn handle<'r>(&self, req: &'r Request, data: Data) -> handler::Outcome<'r> {
        ...
        // verify chain and project
        {
            ...
            if !verifier.CheckLimit(path) {
                ...
                return handler::Outcome::from(
                    req,
                    Json(ApiResp {
                        code: -1,
                        message: "chain or project not match".to_string(),
                        data: None,
                    }),
                );
            }
        ...
        if !self.target.contains_key(&chain) {
            ...
            return handler::Outcome::from(
                ...
                Json(ApiResp {
                    code: -3,
                    message: "chain not config".to_string(),
                    data: None,
                }),
            );
        }
        ...
        if ok {
            ...
            let msg = KafkaInfo {
                ...
                message: parseRequest(
                    req,
                    &resp.data.unwrap_or("null".to_string()),
                    &chain,
                    &pid,
                    &contents,
                    start,
...
fn parseRequest(
    ...
    msg.ip = format!(
        "{:?}",
        req.client_ip()
            .unwrap_or(IpAddr::V4(Ipv4Addr::new(127, 127, 127, 127)))
    );
...
impl Broadcaster {
    pub fn new(
        ...
        Broadcaster {
            sender: KafkaProducerSmol::new(broker, topic),
            rxCh: rx,
        }
    }
